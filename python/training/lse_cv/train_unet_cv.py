...
from blpytorchlightning.loss_functions.CurvatureLoss import CurvatureLoss, CurvatureLoss3D
from blpytorchlightning.loss_functions.MagnitudeGradientSDTLoss import (
    MagnitudeGradientSDTLoss, MagnitudeGradientSDTLoss3D
)
...
def create_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        "--voxel-width", "-vw", type=float, default=61, metavar="VW",
        help="isotropic voxel width, units: um"
    )
    parser.add_argument(
        "--curvature-regularization-threshold", "-crt", type=float, default=0.005, metavar="CRW",
        help="maximum curvature above which excess curvature will be penalized, units: 1/um"
    )
    parser.add_argument(
        "--embedding-conversion-epsilon", "-ece", type=float, default=0.1, metavar="ECE",
        help="scaling parameter for the approximate heaviside function in the embedding conversion"
    )
    parser.add_argument(
        "--lambda-curvature", "-lc", type=float, default=1e-5, metavar="LC",
        help="regularization coefficient for curvature regularization term"
    )
    parser.add_argument(
        "--lambda-mag-grad", "-lmg", type=float, default=1e-5, metavar="LC",
        help="regularization coefficient for magnitude gradient regularization term"
    )
    parser.add_argument(
...
def create_unetplusplus_loss_function(loss_function, classification=True):
    if classification:
        ...
        def unetplusplus_loss_function(y_hat_list):
            loss = 0
            for y_hat in y_hat_list:
                loss += loss_function(y_hat)
            return loss
        return unetplusplus_loss_function


# also need a factory function to create a function to convert lists of embedding fields
def create_unetplusplus_embedding_conversion_function(convert_embeddings):
    def convert_embeddings_unetplusplus(phi_list):
        return [convert_embeddings(phi) for phi in phi_list]
    return convert_embeddings_unetplusplus


# model creation got a bit crazy, separate into function for readability
def create_model(args: Namespace, strategy: str) -> torch.nn.Module():
    model_kwargs = {
...
def train_unet_cv(args: Namespace) -> None:
    ...
    for f in range(args.folds):
        ...
        curvature_loss_function = (
            CurvatureLoss3D(**curvature_kwargs) if args.is_3d else CurvatureLoss(**curvature_kwargs)
        )
        mag_grad_loss_function = (
            MagnitudeGradientSDTLoss3D(args.voxel_width) if args.is_3d else MagnitudeGradientSDTLoss(args.voxel_width)
        )
        convert_embeddings = create_convert_embeddings_to_predictions(args.embedding_conversion_epsilon)

        # if we are doing a UNet++, then the output will be a list of outputs, so we need to redefine all functions
        # that we will be using that take the model output as input
        if args.model_architecture == "unet++":
            classification_loss_function = create_unetplusplus_loss_function(classification_loss_function)
            curvature_loss_function = create_unetplusplus_loss_function(curvature_loss_function, classification=False)
            mag_grad_loss_function = create_unetplusplus_loss_function(mag_grad_loss_function, classification=False)
            convert_embeddings = create_unetplusplus_embedding_conversion_function(convert_embeddings)
        ...
        task = SegmentationEmbeddingTask(
            ...
            lambda_maggrad=args.lambda_mag_grad
        )
