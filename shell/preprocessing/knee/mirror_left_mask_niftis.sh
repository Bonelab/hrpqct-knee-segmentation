...
do
  echo "Mirroring ${i}"
  img="${tibia_dir}${i}L_T_ROI_ALL.nii.gz"
  img_mirror="${tibia_dir}${i}LM_T_ROI_ALL.nii.gz"
...
do
  echo "Mirroring ${i}"
  img="${femur_dir}${i}L_F_ROI_ALL.nii.gz"
  img_mirror="${femur_dir}${i}LM_F_ROI_ALL.nii.gz"
  sbatch --export=IMG="$img",AX="$ax",IMG_MIRROR="$img_mirror" slurm/preprocessing/knee/mirror_image.slurm
  sleep 1
done
