...
then
  ...
  echo "--------------------"
  echo "Example usage: ./projects/preoa/shell/process_image.sh CONMD01R_T tibia right"
  echo ""
  exit
fi
IMAGE=$1
BONE=$2
SIDE=$3
#JID_NII=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} projects/preoa/slurm/0_convert_to_nifti.slurm | tr -dc "0-9")
...
sleep 0.1
JID_CTA=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_PP} projects/preoa/slurm/3_convert_segmentation_to_aims.slurm | tr -dc "0-9")
echo "Submitted job ${JID_CTA} to convert the segmentation to AIMs. Will not execute until job ${JID_PP} is complete."
sleep 0.1
JID_AR=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_PP} projects/preoa/slurm/4_atlas_registration.slurm | tr -dc "0-9")
echo "Submitted job ${JID_AR} to register the image to the atlas. Will not execute until job ${JID_PP} is complete."
sleep 0.1
JID_GR=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_AR} projects/preoa/slurm/5_generate_rois.slurm | tr -dc "0-9")
...
if [ ${BONE} = "femur" ]; then
  ...
  do
    JID_ROI=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE},ROI_CODE=${ROI_CODE} --dependency=afterany:${JID_GR} projects/preoa/slurm/6_convert_roi_to_aim.slurm | tr -dc "0-9")
...
sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_GR} projects/preoa/slurm/7_visualize.slurm
