...
if [ ${BONE} = "femur" ]; then
  ...
  do
    JID_ROI=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE},ROI_CODE=${ROI_CODE} --dependency=afterany:${JID_GR} projects/triknee/slurm/baseline/6_convert_to_aim.slurm | tr -dc "0-9")
