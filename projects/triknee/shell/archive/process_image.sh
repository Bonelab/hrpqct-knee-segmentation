...
sleep 0.1
JID_INF=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_PRE} projects/triknee/slurm/1_inference_unetpp_final.slurm | tr -dc "0-9")
...
sleep 0.1
JID_CTC=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE} --dependency=afterany:${JID_POST} projects/triknee/slurm/4_convert_to_aims.slurm | tr -dc "0-9")
echo "Submitted job ${JID_CTC} to convert the cort/trab masks to AIM format. Will not execute until job ${JID_POST} is complete."
if [ ${BONE} = "femur" ]; then
  for ROI_CODE in 10 11 12 13 14 15 16 17;
  do
    JID_ROI=$(sbatch --export=IMAGE=${IMAGE},BONE=${BONE},SIDE=${SIDE},ROI_CODE=${ROI_CODE} --dependency=afterany:${JID_POST} projects/triknee/slurm/3_convert_to_aim.slurm | tr -dc "0-9")
